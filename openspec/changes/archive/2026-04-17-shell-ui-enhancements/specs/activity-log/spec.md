...
- **THEN** the activity log shows a new entry with the command text and a timestamp or sequence number

#### Scenario: Entry added on command result
- **WHEN** a command completes
- **THEN** the activity log appends the result message (success or error) below the command entry
