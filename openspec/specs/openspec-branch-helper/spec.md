# openspec-branch-helper Specification

## Purpose
Define the OpenSpec helper that creates a branch from the latest change folder.
...
The helper SHALL create a new git branch named after the detected change folder, adding a `feature/` prefix if no supported prefix is already present, and check it out.
